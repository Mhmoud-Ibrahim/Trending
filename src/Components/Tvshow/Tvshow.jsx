...
export default function Tvshow() {
    ...
    const[loading,setLoading]=useState(true);
     async function getTrending(page){
      setLoading(true)
      let {data} = await axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=f3315c4ac6766b68283c8d019253abdc&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=true&page=${page}`)
      setLoading(false)
      seTtv(data.results);
    }
    useEffect(()=>{
...
{loading? <Loading/>:null}
  <div className="row  py-5">
    ...
    {tv.map((item , index)=>  <div  item={item} key={index} className="col-md-2 text-center">
